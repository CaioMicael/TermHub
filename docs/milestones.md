...
| **M1.3** | `packages/daemon/session.ts`: wrapper de `node-pty` (spawn, write, resize, kill, onData, onExit) com ConPTY; `@electron/rebuild` no postinstall | Teste: spawna `pwsh`, escreve `echo hi`, captura `hi` na saída, encerra sem deixar processo órfão |
