...
describe('computePaneDrop', () => {
  ...
  it('rejects a drop with no session being dragged', () => {
    expect(
      computePaneDrop({ sourceSessionId: null, targetSessionId: 2, edge: 'right' }),
    ).toBeNull();
  });
...
describe('encode/decodePaneDragPayload', () => {
  ...
  it('rejects on the MIME type itself, even when the foreign payload would parse as a pane drag', () => {
    // A getData that answers for any type isolates the `types` check: with
    // it gone, this payload would decode to 3 instead of being rejected.
    const dt = { types: ['text/plain'], getData: () => '{"sessionId":3}' };
    expect(decodePaneDragPayload(dt)).toBeNull();
  });
