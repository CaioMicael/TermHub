...
describe('reorderWorkspaceIds', () => {
  ...
  it('clamps an out-of-range insertion index instead of throwing', () => {
    ...
    // -1 is the case the clamp exists for: an unclamped `splice(-1, ...)`
    // counts from the end and would silently land the tab second-to-last.
    expect(reorderWorkspaceIds(['a', 'b', 'c'], 'a', -1)).toEqual(['a', 'b', 'c']);
  });
});
...
describe('encode/decodeTabDragPayload', () => {
  ...
  it('rejects on the MIME type itself, even when the foreign payload would parse as a tab drag', () => {
    // Same isolation as pane-drag.test.ts: getData answers for any type.
    const dt = { types: ['text/plain'], getData: () => '{"workspaceId":"ws-2"}' };
    expect(decodeTabDragPayload(dt)).toBeNull();
  });
